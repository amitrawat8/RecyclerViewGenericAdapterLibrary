...
class ViewTypeAdapter<E : ViewType<*>>(
    ...
    private val bmodel: Int,
    private val bactionItemListener: Int,
    private val bPosition: Int,
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewTypeHolder {
        ...
        return ViewTypeHolder(binding, onItemActionListener, bmodel, bactionItemListener, bPosition)
    }
...
class ViewTypeHolder(
    ...
    fun bindItem(item: ViewType<*>) {
        ...
        binding.setVariable(bmodel, item.data())

        if (item.isUserInteractionEnabled()) {
          binding.setVariable(bactionItemListener, onItemActionListener)
           binding.setVariable(bPosition, bindingAdapterPosition)
        }
        binding.executePendingBindings()
    }
}

//interface onItemclick listerner
